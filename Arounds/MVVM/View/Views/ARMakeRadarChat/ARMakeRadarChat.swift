...
class ARMakeRadarChat: UIView {
    ...
    @objc func keyboardWillHide(notification: NSNotification){
        ...
        UIView.animate(withDuration: 0.2) {
            ...
            self.bottomConstraint.constant = UIScreen.main.bounds.height < 600 ? 130 : 20
            self.layoutIfNeeded()
        }
    }
    ...
    @IBAction func sendChat(_ sender: UIButton) {
        ...
        Database.database().reference().child("radar_messages").observeSingleEvent(of: .value) { (snapshot) in
            if snapshot.exists(), let dict = snapshot.value as? [String: [String: Any]] {
                ...
                let mapped2 = mepped.filter({Date().interval(ofComponent: .day, fromDate: $0) == 0})
                
                if mapped2.count >= 3 {
                    let alert = UIAlertController(title: "Радар: 3 сообщения в сутки", message: "Ты уже отправил 3 сообщения в Радар. Подожди до завтра, чтобы написать снова.", preferredStyle: UIAlertController.Style.alert)
                    alert.addAction(UIAlertAction.init(title: "Договорились", style: .cancel, handler: nil))
                    alert.show()
