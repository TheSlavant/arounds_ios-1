...
extension BaseChatViewController {
    ...
    func setupInputToolbar() {
        ...
        contentView.rightBarButtonContainerView.constraints.filter({$0.firstAttribute == NSLayoutAttribute.height}).first?.constant = height - 14
        contentView.leftBarButtonContainerView.constraints.filter({$0.firstAttribute == NSLayoutAttribute.height}).first?.constant = height - 14
    ...
    private func setupHeaderView() {
        ...
        UIApplication.shared.keyWindow?.bringSubview(toFront: navigationView)
    ...
    func declineSheet() {
        ...
        let block = UIAlertAction(title: "Блок", style: .default, handler: { (action) in
            SVProgressHUD.show()
...
extension BaseChatViewController {
    ...
    override func collectionView(_ collectionView: JSQMessagesCollectionView!, avatarImageDataForItemAt indexPath: IndexPath!) -> JSQMessageAvatarImageDataSource! {
        ...
        if message.senderId == viewModel.me?.id {
            ...
            let image = JSQMessagesAvatarImageFactory.circularAvatarImage(viewModel.me?.getImage() ?? UIImage.init(named: "maleAvatar"), withDiameter: 100)
            return JSQMessagesAvatarImageFactory.avatarImage(with: image, diameter: 100)
        } else {
            var image = viewModel.recever?.getImage()
            if image == nil {
                image = UIImage()
            }
            image = JSQMessagesAvatarImageFactory.circularAvatarImage(image, withDiameter: 100)
            if let cell = collectionView.cellForItem(at: indexPath) as? JSQMessagesCollectionViewCell {
