...
//        myInstaButton.isHidden = !(viewModel?.user?.isMe)!
    ...
    func SetupUI() {
        ...
        let defoultImage = UIImage()
        userImage.image = defoultImage
        
        userImage.kf.setImage(with: user.getImageURL())        
    }
    ...
    func like() {
        if let fromID = ARUser.currentUser?.id,let toID = viewModel?.user?.id {
            if viewModel?.isILike ?? false {
                ...
                FeedBackLogic.shared.likeIsTrue()
    ...
    func chack(callback: @escaping ((Bool,String,Int) -> Void)) {
        ...
        if  block.blockList.contains(viewModel?.user?.id ?? "") {
            callback(false, "Этот пользователь заблокировал вас", 1)
            return
        }
        if  block.blockListByMe.contains(viewModel?.user?.id ?? "") {
    ...
    func blockSheet() {
        ...
        let blockAction = UIAlertAction.init(title: block == true ? "Разблокировать" : "Заблокировать", style: .default, handler: { [weak self] (action) in
            SVProgressHUD.show()
            if block == true {
