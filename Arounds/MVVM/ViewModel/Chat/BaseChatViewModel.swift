...
class BaseChatViewModel {
    ...
    func chack(callback: @escaping ((Bool,String,Int) -> Void)) {
        ...
        if (accepted?.myAccept?.accept ?? .reject) == .reject || block.blockListByMe.contains(accepted?.reciverAccept?.userID ?? "") {
            callback(false, "Вы заблокировали этого пользователя", 2)
            return
        }
        callback(true,"",0)
    }
...
extension BaseChatViewModel {
    ...
    fileprivate func trackLifecycle() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.appBecameInactive(notification: )), name: UIApplication.willResignActiveNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.appBecameActive(notification: )), name: UIApplication.didBecomeActiveNotification, object: nil)
    }
...
extension BaseChatViewModel {
    ...
    func sendMessage(messageId: String,
        ...
        if participantID.isMe(),
            ...
            if accepted?.reciverAccept?.accept != .reject,
                let reciver = recever,
                let current = ARUser.currentUser {
                Database.Inbox.update(chat: chat, sender: current, reciver: reciver, message: jsqMessage)
            }
        }
        ...
        if type != .typing {
            FeedBackLogic.shared.messageIsTrue()
        }
    ...
    func sendMessage(with image: UIImage) {
        ...
        guard let resizedImage = image.resize(toWidth: 320),
    ...
    private func sendMessage(with data: Data, text: String, temporaryUrl: String? = nil, type: ChatMessageType, callBack: ((String?) -> Void)? = nil) {
        ...
        for participant in self.chat.participans {
            ...
            Storage.putData(data, url: "\(Date().timeIntervalSince1970)") {[weak self] (url, errorMessage) in
